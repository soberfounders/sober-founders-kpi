...
import {
    Bot, Sparkles, Calendar, DollarSign, Loader2, Clock,
    ChevronDown, ChevronUp, AlertTriangle, CheckCircle2, Send, RefreshCw,
} from 'lucide-react';
...
function AIBriefingDashboard() {
    ...
    const typeLabel = (type) => BRIEFING_TYPES.find(b => b.mode === type)?.label || type;
    const typeColor = (type) => BRIEFING_TYPES.find(b => b.mode === type)?.color || '#6366f1';
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    ...
                    <div>
                        ...
                        <p style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>
                            Your AI Chief of Staff — strategy briefings, meeting prep, budget optimization, and trend intelligence
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '28px' }}>
                {BRIEFING_TYPES.map((type) => {
                    ...
                    return (
                        ...
                        >
                            {isLoading && (
                                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: `linear-gradient(90deg, transparent, ${type.color}, transparent)`, animation: 'shimmer 1.5s infinite' }} />
                            )}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '10px', backgroundColor: `${type.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {/* ── Error Banner ── */}
            {error && (
                <div style={{ ...baseCardStyle, marginBottom: '20px', backgroundColor: '#fef2f2', borderColor: '#fecaca', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={{ marginBottom: '28px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                    <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'linear-gradient(135deg, #7c3aed, #db2777)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>📊</div>
                    <h3 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--color-dark-green)' }}>Trend Intelligence</h3>
                    <span style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>— 8-week statistical analysis · goal tracking · anomaly detection</span>
                </div>
                {hasSupabaseConfig
                    ? <TrendIntelligencePanel />
                    : <div style={{ ...baseCardStyle, color: 'var(--color-text-secondary)', fontSize: '13px' }}>Supabase config required.</div>
                }
            </div>
            ...
            {activeBriefing && (
                <div style={{ ...baseCardStyle, marginBottom: '28px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                                <span style={{ fontSize: '11px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', color: typeColor(activeBriefing.briefing_type), backgroundColor: `${typeColor(activeBriefing.briefing_type)}15`, padding: '3px 8px', borderRadius: '4px' }}>
                                ...
                                {activeBriefing.confidence != null && (
                                    <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>{Math.round(activeBriefing.confidence * 100)}% confidence</span>
                                )}
                            </div>
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                            {(activeBriefing.delivered_to || []).map((dest) => (
                                <span key={dest} style={{ fontSize: '10px', fontWeight: '600', textTransform: 'uppercase', padding: '3px 8px', borderRadius: '4px', backgroundColor: dest === 'slack' ? '#4a154b15' : '#6366f115', color: dest === 'slack' ? '#4a154b' : '#6366f1' }}>
                    ...
                    {/* Summary */}
                    <div style={{ backgroundColor: '#f8fafc', borderRadius: '10px', padding: '14px', marginBottom: '20px', borderLeft: `3px solid ${typeColor(activeBriefing.briefing_type)}` }}>
                    ...
                    {activeBriefing.anomalies?.length > 0 && (
                        <div style={{ backgroundColor: '#fffbeb', borderRadius: '10px', padding: '14px', marginBottom: '20px', borderLeft: '3px solid #d97706' }}>
                    ...
                    {activeBriefing.projected_impact && (
                        <div style={{ backgroundColor: '#f0fdf4', borderRadius: '10px', padding: '14px', marginBottom: '20px', borderLeft: '3px solid #16a34a' }}>
                    ...
                    {(activeBriefing.sections || []).map((section, idx) => (
                        <div key={idx} style={{ marginBottom: '18px' }}>
                            <h4 style={{ fontSize: '14px', fontWeight: '700', color: 'var(--color-text-primary)', marginBottom: '8px' }}>{section.heading}</h4>
                    ...
                    {(activeBriefing.action_items || []).length > 0 && (
                        <div style={{ marginTop: '20px', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
                            <h4 style={{ fontSize: '14px', fontWeight: '700', color: 'var(--color-text-primary)', marginBottom: '12px' }}>Action Items</h4>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {activeBriefing.action_items.map((item, i) => {
                                    const ps = priorityStyles[item.priority] || priorityStyles.medium;
                                    return (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '8px', backgroundColor: ps.bg, border: `1px solid ${ps.border}` }}>
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                                                    ...
                                                    {item.assignee && <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>→ {item.assignee}</span>}
                                                </div>
                                            </div>
                                            <button
                                ...
                                <div key={item.id}>
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)', whiteSpace: 'nowrap' }}>{formatDate(item.created_at)}</span>
