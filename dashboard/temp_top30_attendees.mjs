...
function sourceBucketFromLumaEvidence(ev) {
  ...
  if (ots && ots !== 'NOT FOUND') {
    const m = { PAID_SOCIAL:'Paid Social (Meta)', ORGANIC_SEARCH:'Organic Search', REFERRALS:'Referral', DIRECT_TRAFFIC:'Direct', EMAIL_MARKETING:'Email', PAID_SEARCH:'Paid Search', SOCIAL_MEDIA:'Social (Organic)' };
    return { bucket: m[ots] || ots.replace(/_/g, ' '), method: 'Luma HubSpot Original Source' };
  }
  const heard = String(ev.hearAboutCategory || '');
...
const hubspotRows = await pagedSelect('raw_hubspot_contacts', 'hubspot_contact_id,createdate,email,firstname,lastname,annual_revenue_in_dollars,annual_revenue_in_dollars__official_,sobriety_date,hs_analytics_source,hs_analytics_source_data_1,hs_analytics_source_data_2,campaign', 'createdate', `${startKey}T00:00:00.000Z`);
const lumaRows = await pagedSelect('raw_luma_registrations', 'guest_name,guest_email,event_date,event_start_at,registered_at,approval_status,is_thursday,registration_answers,matched_zoom', 'event_date', startKey);
...
for (const row of hubspotRows) {
  ...
  if (em) { if (!hubspotByEmail.has(em)) hubspotByEmail.set(em, []); hubspotByEmail.get(em).push(row); }
}
...
const top30 = Array.from(history.values()).map((h) => {
  ...
  const useFallback = (contactBucket === 'Unknown' || contactBucket === 'Other') && lumaFallback.bucket !== 'Unknown';
  const sourceBucket = useFallback ? lumaFallback.bucket : contactBucket;
  const sourceAttributionMethod = useFallback ? (contact ? `HubSpot Unknown -> ${lumaFallback.method}` : lumaFallback.method) : (contact ? 'HubSpot Original Source' : 'Unattributed');
  return {
...
console.log(JSON.stringify({ lookbackDays: LOOKBACK_DAYS, startKey, totalZoomSessionsRows: zoomFiltered.length, totalUniqueAttendees: history.size, top30 }, null, 2));
