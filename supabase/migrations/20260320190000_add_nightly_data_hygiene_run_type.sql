-- Add 'nightly_data_hygiene' to hubspot_sync_runs run_type CHECK constraint
-- and update the observability view to include it.

-- 1) Replace the CHECK constraint to include the new run type
ALTER TABLE public.hubspot_sync_runs
  DROP CONSTRAINT IF EXISTS hubspot_sync_runs_run_type_check;

ALTER TABLE public.hubspot_sync_runs
  ADD CONSTRAINT hubspot_sync_runs_run_type_check
  CHECK (run_type IN (
    'webhook_worker',
    'incremental',
    'reconcile_hourly',
    'reconcile_daily',
    'backfill',
    'nightly_data_hygiene'
  ));

-- 2) Recreate the observability view with the new run type
...
expected AS (
  ...
  UNION ALL
  SELECT 'nightly_data_hygiene'::text, NULL::text
),
runs_normalized AS (
  SELECT
    r.id,
...
SELECT
  ...
  CASE
    ...
    WHEN ex.run_type = 'nightly_data_hygiene' THEN ls.last_success_at < (NOW() - INTERVAL '30 hours')
    ELSE false
  END AS is_stale,
  q.pending_events,
  q.processing_events,
...
ORDER BY
  CASE ex.run_type
    ...
    WHEN 'nightly_data_hygiene' THEN 6
    ELSE 99
  END,
